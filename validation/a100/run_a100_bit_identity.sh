#!/usr/bin/env bash
# ===========================================================================
# validation/a100/run_a100_bit_identity.sh — K5 A100 (sm_80) Cobalt job
# ===========================================================================
# Rebuilds against $HOME/kokkos-install-cuda-sm80-quadmath and runs
# bit_identity_test on the allocated GPU. Wrapper and core are both evaluated
# inside the same Cuda kernel (see tests/bit_identity_test.cpp header).
#
# SUBMIT:
#     qsub -A pepper_hep -n 1 -t 180 -q gpu_a100 --mode script \
#          validation/a100/run_a100_bit_identity.sh
#
#COBALT -A pepper_hep
#COBALT -n 1
#COBALT -t 180
...
BUILD_DIR=${BUILD_DIR:-$REPO_ROOT/build-a100-bit-identity}
...
LOG="$LOGDIR/a100_bit_identity_${STAMP}.log"

exec > >(tee -a "$LOG") 2>&1

echo "==========================================================="
echo " A100 bit-identity (K5)"
echo " date      : $(date -Is)"
echo " host      : $(hostname)"
echo " repo      : $REPO_ROOT"
...
CXX=${CXX:-$KOKKOS_PREFIX/bin/nvcc_wrapper}
export CXX
