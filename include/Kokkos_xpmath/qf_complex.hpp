...
//
// Kokkos::complex<QuadFloat> does not instantiate (Kokkos 5.1
...
// ============================================================
// Kokkos::atomic_add — QuadFloatComplex (componentwise on re/im)
// ============================================================
// Each component uses the real-type CAS over its full expansion (see
// qf_math.hpp / impl/atomic_cas_add.hpp). Determinism caveat applies per
// component: order-dependent last-limb differences are not a defect.
namespace Kokkos {
KOKKOS_INLINE_FUNCTION void atomic_add(Experimental::QuadFloatComplex* dest,
                                       Experimental::QuadFloatComplex const& val) {
  Kokkos::atomic_add(&dest->re, val.re);
  Kokkos::atomic_add(&dest->im, val.im);
}
KOKKOS_INLINE_FUNCTION Experimental::QuadFloatComplex atomic_fetch_add(
    Experimental::QuadFloatComplex* dest,
    Experimental::QuadFloatComplex const& val) {
  Experimental::QuadFloatComplex old;
